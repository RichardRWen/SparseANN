...
int main(int argc, const char **argv) {
    if (argc < 8) {
        std::cout << "Usage: " << argv[0] << " [num points (1000000)] [num queries (10000)] [dimensions (128)] [clusters (10)] [standard deviation (0, 0.5)] [base outfile (base.fbin)] [query outfile (queries.fbin)]" << std::endl;
        return 0;
    }
    ...
    std::vector<std::vector<float>> point_set = generate_points_gaussian_mixture(num_points + num_queries, dimensions, initial_points, clustering_factor);
