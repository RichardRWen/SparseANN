...
int main(int argc, const char **argv) {
    if (argc < 7) {
        std::cout << "Usage: " << argv[0] << " [num points (1000000)] [num queries (10000)] [dimensions (128)] [clustering factor (1, 100)] [base outfile (base.fbin)] [query outfile (queries.fbin)]" << std::endl;
        return 0;
    }
    ...
    float clustering_factor = std::stof(argv[4]);  // Control how strongly new points prefer to connect to existing clusters
    const char *base_filename = argv[5];
    const char *query_filename = argv[6];

    parlay::sequence<std::vector<float>> point_set = generate_points_knn(num_points + num_queries, dimensions, 10, clustering_factor);
