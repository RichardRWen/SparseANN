/*
====IMPORTANT====
This program does not work for Euclidean distance
*/
...
int main(int argc, char **argv) {
	if (argc < 4) {
		std::cout << "Usage: " << argv[0] << " [path to inserts .bin] [path to queries .bin] [path to outfile] [optional number of queries]" << std::endl;
		exit(0);
	}
	...
	forward_index<float> inserts(argv[1], "bin");
	...
	forward_index<float> queries(argv[2], "bin", num_queries_to_read);
